...
public class UniversalEnumEditor : EditorWindow
{
    ...
    {
        // This new method is much more robust. It finds the exact code block for the enum.
        string scriptContent = script.text;

        // This pattern will find the enum definition and capture the content between its braces { }.
        // It correctly handles different spacing and newlines.
        ...
        // We get only the content INSIDE the braces.
        string enumContent = match.Groups[1].Value;

        // Now we parse the content inside the block.
        return enumContent.Split(',')
            .Select(v => v.Trim())
            .Where(v => !string.IsNullOrEmpty(v) && !v.StartsWith("//")) // Ignore empty entries and comments
            .Select(v => v.Split('=')[0].Trim()) // Handle explicit values like "Value = 5"
            .Where(v => IsValidEnumName(v)) // Final validation
            .ToArray();
    }
    ...
    private void UpdateEnumFile(TextAsset script, string enumName, string[] newValues)
    {
        ...
        // Build the new content for inside the braces.
        ...
        // Use the captured groups to precisely replace only the content inside the enum.
    ...
    private void DrawSearchAndSortControls() { EditorGUILayout.BeginHorizontal(); searchQuery = EditorGUILayout.TextField(searchQuery, searchFieldStyle, GUILayout.ExpandWidth(true)); if (GUILayout.Button("", searchCancelButtonStyle)) { searchQuery = ""; GUI.FocusControl(null); } sortAlphabetically = GUILayout.Toggle(sortAlphabetically, new GUIContent(" Sort A-Z", EditorGUIUtility.IconContent("AlphabeticalSorting").image), EditorStyles.miniButton, GUILayout.Width(80)); EditorGUILayout.EndHorizontal(); FilterEnums(); EditorGUILayout.Space(5); }
